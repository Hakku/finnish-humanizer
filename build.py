...
# Manually maintained files NOT generated by build_platforms().
# Checked by check_manual_sync() at the end of main().
MANUAL_FILES = [
...
def _count_patterns() -> int:
    """Count pattern sections (### N.) in patterns.md."""
...
def check_manual_sync(pattern_count: int) -> list:
    ...
    for path in MANUAL_FILES:
        ...
        for m in stale_re.finditer(text):
            if int(m.group(1)) != pattern_count:
                try:
...
def main():
    ...
    if sync_warnings:
        for w in sync_warnings:
            print(w)
        print(f"    -> Päivitä nämä tiedostot: {pattern_count} patternia")
        sys.exit(1)
    print(f"  OK — {pattern_count} patternia, manuaalitiedostot ajan tasalla")
